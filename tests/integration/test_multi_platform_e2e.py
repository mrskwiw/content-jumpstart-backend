...
class TestMultiPlatformE2E:
    ...
    async def test_facebook_generation_and_validation(
        ...
        """Test Facebook content generation (50-80 words) + validation"""
        # Step 1: Parse brief
        brief_text = sample_brief_file.read_text()
        brief = brief_parser.parse_brief(brief_text)
        ...
        # Check that posts are within the medium-length target
        if not all(40 <= p.word_count <= 100 for p in posts):
            print("⚠️ WARNING: Facebook posts outside 40-100 word range!")
            print(f"Word counts: {[p.word_count for p in posts]}")
            print("Target: 50-80 words, Min: 40 words, Max: 100 words")
