...
def test_submit_recovers_from_insert_conflict(db_session, monkeypatch):
    ...
    # Recovered from the insert conflict onto the winner's row (no 500), and the submit
    # reopened review (pending) — resubmit always resets. Single row.
...
def test_submit_reopens_review_on_approved_post(db_session):
    # Decision #215: resubmitting an already-approved post reopens review (→ pending), so
    # edited-after-approval content can't ship on a stale approval.
    client = TestClient(app)
