...
class TestPlatformAwarePrompts:
    ...
    def test_linkedin_prompt_construction(self, generator, sample_brief):
        ...
        assert "220-280 words" in prompt
    ...
    def test_twitter_prompt_critical_warnings(self, generator, sample_brief):
        ...
        assert "every word count" in prompt.lower()
    ...
    def test_facebook_prompt_critical_warnings(self, generator, sample_brief):
        ...
        assert "50-80 words" in prompt or "40 words" in prompt

        # Should include some enforcement indicator
        assert "📘" in prompt or "CRITICAL" in prompt

        # Should warn about completeness requirements
        assert "COMPLETE" in prompt or "FAIL" in prompt or "HARD LIMIT" in prompt
    ...
    def test_blog_prompt_structure_requirements(self, generator, sample_brief):
        ...
        assert "200-300 words" in prompt  # Introduction section word count

        # Should specify sections (updated from Body)
        assert "Section" in prompt  # Blog has multiple sections now
        assert "300-400 words" in prompt  # Section word count
    ...
    def test_prompt_length_reasonable(self, generator, sample_brief):
        ...
        for platform in platforms:
            ...
            if platform == Platform.BLOG:
                assert (
                    len(prompt) < 17000
                )  # Still reasonable (updated to accommodate expanded blog requirements)
            else:
                assert (
                    len(prompt) < 15000
                )  # Not too long for other platforms (increased to accommodate content-creator skill injection)
