...
class TestHookValidatorPlatform:
    ...
    def test_facebook_hook_over_80_chars(self, validator):
        """Test Facebook hook over 120 chars fails"""
        long_hook = "C" * 130  # 130 characters
        posts = [
            Post(
                content=long_hook,
                template_id=1,
        ...
        assert result["platform"] == "facebook"
        assert len(result["hook_length_issues"]) == 1
        assert result["hook_length_issues"][0]["hook_length"] == 130
        assert result["hook_length_issues"][0]["max_allowed"] == 120
        assert result["passed"] is False
