...
def test_seo_keywords_empty_list_treated_as_none():
    """Test SEO keywords treats empty list as None (auto-generate from business profile).

    Bug #64 fix: frontend sends [] when user leaves topics blank; this should
    trigger auto-generation instead of raising a 422 validation error.
    """
    params = SEOKeywordParams(main_topics=[])
    assert params.main_topics is None


def test_seo_keywords_too_many():
    """Test SEO keywords silently coerces >10 topics down to 10."""
    params = SEOKeywordParams(main_topics=[f"topic{i}" for i in range(11)])  # 11 topics
    assert len(params.main_topics) == 10  # truncated, not rejected


def test_seo_keywords_topic_too_short():
    """Test SEO keywords silently skips topics shorter than 3 chars."""
    params = SEOKeywordParams(main_topics=["AI", "OK", "NO", "valid topic"])
    # 2-char topics are skipped; only the valid one survives
    assert len(params.main_topics) == 1
    assert params.main_topics[0] == "valid topic"


def test_seo_keywords_topic_too_long():
    """Test SEO keywords truncates topics longer than 100 chars."""
    long_topic = "A" * 101
    params = SEOKeywordParams(main_topics=[long_topic])
    assert len(params.main_topics) == 1
    assert len(params.main_topics[0]) == 100  # truncated to max
...
def test_content_gap_empty_valid():
    """Test content gap accepts empty topics (auto-generates from SEO keywords or business profile)."""
    params = ContentGapParams(current_content_topics="")
    assert params.current_content_topics == ""

    # Also test with default (no argument)
    params2 = ContentGapParams()
    assert params2.current_content_topics == ""


def test_content_gap_too_short_when_provided():
    """Test content gap rejects topics that are too short when provided (min 10 chars if not empty)."""
    with pytest.raises(ValidationError) as exc_info:
        ContentGapParams(current_content_topics="short")  # Only 5 chars (min is 10 when provided)

    errors = exc_info.value.errors()
    assert any("too short" in str(error["msg"]).lower() for error in errors)


def test_content_gap_minimum_length_when_provided():
    """Test content gap accepts exactly 10 characters when provided."""
    params = ContentGapParams(current_content_topics="A" * 10)  # Exactly 10 chars
    assert len(params.current_content_topics) == 10
