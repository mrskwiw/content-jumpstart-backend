...
def test_content_gap_analysis_validation():
    ...
    # Test empty current topics is now VALID (auto-generates from SEO keywords or business description)
        ...
        analyzer.validate_inputs(
            {
                ...
                "current_content_topics": [],  # Empty is now valid - will auto-generate
            }
        )
        print("  ✓ Empty current_content_topics accepted (will auto-generate)")
    except ValueError:
        pytest.fail("Empty current_content_topics should be valid (auto-generates)")
