...
class Deliverable(Base):
    ...
    )  # NULL for research-only reports
    ...
    )  # draft, ready, delivered - indexed for filtering
    ...
    __table_args__ = (
        ...
        Index(
            "ix_deliverables_client_status", "client_id", "status"
        ),  # Filter by client AND status
        Index(
            "ix_deliverables_project_status", "project_id", "status"
        ),  # Filter by project AND status
        Index(
            "ix_deliverables_status_created", "status", "created_at"
