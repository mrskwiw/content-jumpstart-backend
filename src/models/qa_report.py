...
class QAReport(BaseModel):
    ...
    def to_markdown(self) -> str:
        ...
        # Render ONLY a complete payload: an incomplete one (e.g. a deserialized legacy report)
        # is omitted rather than crashing this user-visible path OR silently misreporting missing
        # counts as zero (which would falsely claim "no weak openings" in a QA artifact).
        ab = self.answer_block_geo
        if ab and {"total", "ok_count", "weak_count"} <= ab.keys() and ab["total"]:
